...
class RingDetector(Node):
    ...
    def extract_ring_color(self, cv_image, e1, e2):
        # Create a mask with the same size as the image
        mask = np.zeros(cv_image.shape[:2], dtype=np.uint8)

        # Draw the larger ellipse on the mask
        cv2.ellipse(mask, e1, (255, 255, 255), -1)

        # Draw the smaller ellipse on the mask with black color
        cv2.ellipse(mask, e2, (0, 0, 0), -1)

        # Apply the mask to the original image to get the ring area
        ring_area = cv2.bitwise_and(cv_image, cv_image, mask=mask)

        # Convert the ring area to HSV
        hsv_ring_area = cv_image

        # Define color ranges for red, green, blue, and black in HSV
        color_ranges = {
        ...
        # Loop over the color ranges to find the ring color
        for color, (lower, upper) in color_ranges.items():
            mask = cv2.inRange(hsv_ring_area, np.array(lower), np.array(upper))
            if np.any(mask):
                return color
    ...
    def image_callback(self, data):
        ...
        self.elipses = []
        self.image_data = data  # Save the image data
        try:
        ...
        for c in candidates:
            ...
            self.ring_color = self.extract_ring_color(cv_image, e1, e2)
                ...
                if valid_points:
                    ...
                    if ring_point_map.z > -0.03 and self.is_new_ring(ring_point_map):
                        ...
                        if self.ring_color != "unknown":
                            self.speak(self.ring_color)

                        print(self.ring_color)
